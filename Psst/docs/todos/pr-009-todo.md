...
- [x] Create branch `feat/pr-009-basic-media-support-and-image-messaging` from develop
...
- [x] Extend Message model with media fields
  - Test Gate: Message model compiles with new fields
- [x] Add mediaType, mediaURL, mediaThumbnailURL, mediaSize, mediaDimensions to Message struct
  - Test Gate: All fields are optional and properly typed
- [x] Update CodingKeys to exclude media fields from Firestore serialization (client-side only)
  - Test Gate: toDictionary() method includes media fields
- [x] Update toDictionary() method to include media fields for Firestore writes
...
- [x] Create ImagePickerView.swift
  - Test Gate: SwiftUI Preview renders; zero console errors
- [x] Implement SwiftUI wrapper for PHPickerViewController
  - Test Gate: Image picker presents correctly
- [x] Add camera and photo library access
  - Test Gate: Both camera and library options work
- [x] Implement image selection callback
  - Test Gate: Selected images return to parent view
- [x] Create ImageMessageView.swift
  - Test Gate: SwiftUI Preview renders; zero console errors
- [x] Implement async image loading with proper error handling
...
- [x] Implement proper aspect ratio and sizing
  - Test Gate: Images display with correct proportions
- [x] Update MessageRow.swift to display images
  - Test Gate: SwiftUI Preview renders; zero console errors
- [x] Add conditional logic to display ImageMessageView when mediaType == "image"
  - Test Gate: Image messages show ImageMessageView, text messages show Text
- [x] Keep existing text message display logic intact
  - Test Gate: Text messages continue to work as before
- [x] Update ChatView.swift message input
  - Test Gate: SwiftUI Preview renders; zero console errors
- [x] Add camera icon button next to send button
  - Test Gate: Camera button appears and is tappable
- [x] Integrate ImagePickerView sheet presentation
...
- [x] Configure storage path structure: /chat-images/{chatID}/{messageID}.jpg
  - Test Gate: Images save to proper folder structure
- [x] Set up thumbnail path: /chat-images/{chatID}/thumbnails/{messageID}_thumb.jpg
  - Test Gate: Thumbnails save to thumbnail subfolder
- [x] Real-time listeners working for image messages
  - Test Gate: Image messages sync across devices <100ms
- [x] Offline persistence for image messages
...
- [x] Image display renders in <1 second
  - Test Gate: Images appear quickly after upload completion
- [x] Image compression reduces file size by >50%
  - Test Gate: Large images compressed significantly
- [x] Smooth 60fps scrolling with image messages
  - Test Gate: Chat scrolls smoothly with mixed text/image messages
- [x] App load time < 2-3s (unchanged with image support)
